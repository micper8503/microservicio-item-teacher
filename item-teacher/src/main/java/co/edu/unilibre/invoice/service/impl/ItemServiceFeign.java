...
public class ItemServiceFeign implements IItemService{
	...
	@Autowired
	private ProductServiceClientFeign teacherServiceClientFeign; 
	
	@Override
	public List<Item> getAll() {
		List<Product> products = teacherServiceClientFeign.getList();
		return products.stream().map(prod -> new Item(prod, 1)).collect(Collectors.toList());
	}

	@Override
	public Item findById(Long id, Integer quantity) {
		return new Item(teacherServiceClientFeign.getById(id), quantity);
	}
